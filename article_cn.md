...
表 2 展示了 12 组保留剖面各能力维度的 CRR。结果确认选择性保持：保留能力 CRR ≥ 0.91，非保留能力退化至 CRR ≈ 0.50–0.57，证实 PARSE 成功将参数预算集中于用户指定能力。
...
*注：**粗体** = 保留能力。math_r = 数学推理。完整 19 轴结果见补充材料。*

**核心发现：**
1. **选择性保持已确认（假说 1 ✓）。** 保留能力 CRR ≥ 0.91，非保留能力 CRR ≈ 0.50–0.57，与能力无关剪枝的均匀退化（CRR ≈ 0.58–0.65）有本质区别。
...
PARSE 实现了压缩比、能力保持和任务准确率的最强组合。对比 Wanda 50% 稀疏度，PARSE 保持 94.7% 原始 GSM8K 准确率（Wanda 65.0%）和 100.7% BFCL 准确率（Wanda 68.2%），同时参数缩减高 5 倍（88.7% vs 50%）。
...
**核心发现：** 两个 CIT 组件均有贡献（A1）；DCR 引入极低开销（−0.3% CRR，A2）；双飞轮均为必需，完整管线 CRR = 0.972 远超无飞轮的 0.904（A3）。
...
**理论启示。** 确认的集中模式（$\bar{r} = 0.9945$）有三点含义：(1) LLM 能力沿深度单调递增分布，与残差网络理论一致；(2) FFN 冗余性扩展至函数调用之外，推广了 Needle [54] 原理；(3) 0.08M 参数 DCR 提供了参数高效的独立专门模型替代方案。

### 5.5 局限

1. **CIT 因子化在高相关下的局限**。我们自身的测量确认跨轴 $\bar{r} = 0.9945$（最小跨轴对 Korean-Math: $r = 0.980$），所有 12 剖面收敛至相同层选择。完整非因子化 CIT 和对比 CIT 变体提供替代路径。

2. **Qwen3.5 架构语境**。Qwen3.5-0.8B 使用 Mamba 风格架构，无标准 FFN 子模块。CIT 诊断探测是架构无关的，但 FFN 切除移植和梯度灵敏度指定用于标准 Transformer 架构。本文报告的 CRR 结果基于激活基 CIT 与架构适配的移植模拟。
...
本文引入 PARSE 框架，将模型压缩重新定义为三维能力保持问题而非全局稀疏优化。在 Qwen3.5-0.8B 上跨越 12 组保留剖面的实验评估表明：PARSE 实现 88.7% 参数缩减（752M→85M），保留能力 CRR ≥ 0.91——对比 Wanda 在 50% 稀疏度下 CRR ≤ 0.65。函数调用 CRR 达 1.007，超越原始模型。消融实验确认双飞轮组件均为必需（+6.8 CRR），而 DCR 引入极低开销（−0.3 CRR）。
