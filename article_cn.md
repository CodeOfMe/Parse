...
本文提出的 **PARSE** 框架，以"该保留什么、该替换什么"的精准外科思维，重新定义了模型压缩的目标函数——从全局稀疏优化转变为三维能力保持问题。在 Qwen3.5-0.8B 基座上，PARSE 以 8.8× 压缩比保持了超过 95% 的目标能力精度，动态能力路由器以 0.08M 参数的代价实现了 12 组保留剖面的统一服务。核心理念简洁而有力：**在极小模型的世界里，手术刀的精确度远比铁锤的力度更为有效。**

PARSE 的做法不同于现有方向：不是从零训练专精模型，也不是对已有模型一刀切地剪枝，而是在预训练模型的参数空间内，通过能力分解张量识别出对指定任务关键的结构并保留，仅替换冗余部分。这种策略的工程含义是明确的——如果不关心函数调用，相关层可以极致压缩；如果需要保留数学推理，相关层的关键权重一个不能动。压缩比与能力保留率之间的 trade-off 由保留剖面的大小决定，而非由预定义的全局稀疏度决定。
