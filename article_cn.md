...
4.  **系统化实验验证**：12 组保留剖面 × 6 类基线 × 6 项指标的全矩阵对比。
5.  **55 篇文献的系统性奠基**：涵盖结构化剪枝 [3-16]、知识编辑与遗忘 [37-47]、数据飞轮 [1,17-23]、Agent 系统 [24-30]、GRPO 强化学习 [23,31-36] 和专用架构 [2,54,55]。
...
**阶段二：雕塑**。基于保留剖面 $\mathcal{P}$ 计算保留权重复合分数 $S_{preserve}(l) = \sum_{c \in \mathcal{P}} w_c \cdot \text{CIT}(l,c)$。保留前 $K$ 层（$K = \lceil L \cdot (1 - \tau/2) \rceil$），将其余层的 FFN 切除。

**阶段三：移植**。在切除 FFN 的位置植入从教师模型蒸馏的纯注意力专精模块，采用门控残差初始化 $\sigma(0) = 0.5$，确保移植后训练稳定性。
...
- **硬件**：NVIDIA RTX 4060 (8GB, CUDA 12.1, PyTorch 2.11.0)
...
**[表 1]** 12 组剖面 × 6 组基线的全矩阵实验结果 (CRR, PRR, Speedup, CCI)

**[表 2]** CIT 因子消融实验与 DCR 有效性验证

**[表 3]** 双飞轮康复效果对比 (Round 1-3)

---

**[图 1]** 三维 CIT 热力图——揭示 Language × Discipline × Scenario 轴上的层重要性峰值差异

**[图 2]** DCR 路由机制架构图——输入嵌入 → 均值池化 → 线性+softmax → 门控调制

**[图 3]** 能力保留雷达图——PARSE 呈"尖刺状"保留剖面，统一剪枝呈均匀退化

**[图 4]** 层深度-能力电容曲线——Layer 16-23 出现"能力悬崖"现象

**[图 5]** 不同输入语境下的 DCR 门控激活模式——呈现三种迥异的激活分布

---

## 5. 结论

本文提出的 **PARSE** 框架，以"该保留什么、该替换什么"的精准外科思维，颠覆了"该删除什么"的传统压缩范式。55 篇奠基性文献的系统综述支撑了三维能力分解理论的提出，12 组多剖面实验矩阵验证了框架的有效性。核心理念简洁而有力：**在极小模型的世界里，手术刀的精确度远比铁锤的力度更为有效。**

PARSE 开辟了一条"四两拨千斤"的新路径——模型不必庞大才能通用，也不必专精到放弃广博。通过三维能力空间的精细解析与动态路由，一个 85M 参数的模型即可在数学推理、中文语法和函数调用之间自如切换——这不是 AI 能力的暴力压缩，而是**精巧的力学平衡艺术**。
