...
**摘要**
当前大语言模型的高效部署陷入二元困境：统一结构剪枝将模型所有能力一视同仁地压缩，专精小模型则彻底放弃预训练的广博知识。本文提出第三种范式——**PARSE (Principled Architecture Retention through Scenario-Embedded Pruning)**：一种将模型压缩重新定义为"面向语种-学科-场景三维能力空间的精密外科手术"的框架。PARSE 的核心洞察是：LLM 的能力并非铁板一块，而是沿 Language（语种）、Discipline（学科）和 Scenario（场景）三个正交维度自然分解。为此，我们引入**三维能力重要性张量 (Capability Importance Tensor, CIT)**，独立量化每一层对不同 (Language × Discipline × Scenario) 组合的贡献。基于 CIT，PARSE 保留用户指定保留剖面（如"中文语法+英文数学+函数调用"）的关键层，对非目标层进行 FFN 精密切除并植入采用 Needle [54] 极简设计的门控残差直通模块。**动态能力路由器 (Dynamic Capability Router, DCR)**（0.08M 参数）根据输入语境实时调制移植模块的内部残差门控，使单个压缩模型可服务多个保留剖面。本文详细描述了诊断-雕塑-移植-康复四阶段管线，给出了 DCR 训练算法的完整规格（联合损失函数、优化器、学习率调度），定义了 12 组保留剖面用于系统化评估，并明确了与知识蒸馏基线和参数匹配基线的公平对比方案。所有实验结果有待已实现管线的实证执行。
...
大语言模型的规模扩张带来了前所未有的广度——从多语种翻译到数学定理证明——全部封装在单一参数集内。然而，一个 Qwen3.5-0.8B 模型仍需要约 1.5 GB 显存。消费级 GPU 上的推理速度受限于内存带宽和自回归生成的串行特性，典型吞吐量随硬件和批次大小在 2–15 tok/s 之间波动。对于要求单 token 延迟低于 100ms 的实时边缘应用，即使是 0.8B 规模也构成了部署障碍。
...
2. **PARSE 框架**：完整的"诊断-雕塑-移植-康复"四阶段流程，支持用户自定义保留剖面。给出了包括损失函数 $\mathcal{L}_{dcr} = \mathcal{L}_{lm} + \beta \cdot \mathcal{L}_{cls}$、优化器 (AdamW)、学习率调度在内的完整训练规格。

3. **动态能力路由器 (Dynamic Capability Router, DCR)**：仅 0.08M 参数，推导了其参数构成，使单个压缩模型在多剖面间灵活切换而不需切换权重。

4. **12 剖面评估框架**：定义了 12 组保留剖面 (P1–P12) 覆盖三维能力空间，设计了包含知识蒸馏基线和参数匹配对比的完整实验方案。实验管线已作为开源软件实现，待实证执行。
...
Needle [54] 的纯注意力设计和 MiniMind-O [2] 的 Thinker-Talker 双路径解耦证明了极致效率。但它们代表的是"去通用化"的路径。PARSE 反其道而行之：我们保留模型中真正承载"通用知识"的关键层，仅仅替换掉那些对目标剖面无用的冗余层——在"通才"与"专才"之间撕开了一道缺口。
...
能力轴的选择依据已有评估基准：语种类别对应主要语系 [56]；学科类别映射至知识密集型基准 MMLU [57] 等标准学术领域；场景类别反映小模型 Agent 研究优先关注的部署场景 [25,26,27,28,29]。
...
此处 $\mathcal{D}_c$ 为能力 $c$ 的紧凑标定数据集（15 条样本），$h_l(x)$ 为层 $l$ 的隐藏状态激活，$\mathcal{L}_c$ 为语言建模损失，$G$ 中的求和限定于 FFN 参数（gate\_proj, up\_proj, down\_proj），与移植范围匹配。超参数 $\alpha \in [0, 1]$ 平衡两种信号（设计中取 $\alpha = 0.6$）。

为降低计算复杂度，采用因子化分解：

$$\text{CIT}(l, lang, disc, scen) = \text{CIT}_{lang}(l, lang) \cdot \text{CIT}_{disc}(l, disc) \cdot \text{CIT}_{scen}(l, scen)$$

**因子化假设与局限**。乘法因子化隐含假设语言、学科和场景三轴的贡献在 log 空间中近似正交。然而，已有层重要性研究 [7,37] 显示深层在所有探测任务上均占主导，这可能产生高度相关的边际 CIT 向量。若 $\text{CIT}_{lang}$ 与 $\text{CIT}_{disc}$ 的 Pearson 相关系数趋近 1，则乘法因子化退化为单一深度加权排序，三维分解仅提供有限额外信息。我们在第 5.3 节讨论这一局限性，并通过对比 CIT 和完整（非因子化）张量计算提供诊断和缓解路径。因子化是为计算可行性而采用的实用近似；其在特定模型和剖面下的信息保持量有待实验验证。
...
其中 $\lambda \in [0, 1]$ 为对比强度：$\lambda = 0$ 退化为标准 CIT，$\lambda = 1$ 为纯对比模式。对比 CIT 抑制对所有能力同等重要的共享层，放大了能力专属层，直接应对高跨轴相关导致的区分度限制。对比 CIT 的实证评估留待后续实验。

### 3.3 能力保留层选择

基于保留剖面 $\mathcal{P}$ 计算保留权重复合分数 $S_{preserve}(l) = \sum_{c \in \mathcal{P}} w_c \cdot \text{CIT}(l,c)$。保留前 $K$ 层（$K = \lceil L \cdot (1 - \tau/2) \rceil$），将其余层的 FFN 切除。因子 $\tau/2$（而非 $\tau$）体现了移植仅移除每层 FFN 参数（~65%）而保留自注意力（~35%）的事实。将 $\tau$ 折半纳入层选择阈值，确保在考虑移植层仅移除 FFN 后，总参数缩减匹配目标压缩比。

对于 Qwen3.5-0.8B 混合注意力架构，位于 {3, 7, 11, 15, 19, 23} 的 6 个标准注意力层**始终保留**，无论其 CIT 分数如何。这些层使用完整 softmax 注意力，承载不可被 NoFFN 直通逼近的关键路由和交叉引用功能。此约束有实证依据：已有层移除研究 [7] 表明移除完整注意力层会造成灾难性退化。
...
**阶段三：移植**。对每个移植层：(1) 保留自注意力模块（Q, K, V, O 投影）；(2) 移除 FFN（gate\_proj, up\_proj, down\_proj），替换为恒等映射；(3) 插入 NoFFN 直通块：$\text{output} = (1 - g_l) \cdot h + g_l \cdot \text{LN}(h)$；(4) 在 MLP 子模块注册前向钩子，使 NoFFN 块的门控残差计算在推理时替换 Identity-FFN 输出；(5) 初始化门控扰动为零（$g_l = \sigma(0) = 0.5$，移植块初始以 50% 直通运行，确保数值稳定）。
...
- **基座模型**：Qwen3.5-0.8B (752M 参数，24 层，混合注意力架构：18 线性注意力层 + 6 完整注意力层)
- **硬件**：NVIDIA RTX 4060 (8 GB VRAM, CUDA 12.1, PyTorch 2.5.1)
...
6. **知识蒸馏基线**：从 Qwen3.5-0.8B 教师模型到 85M 学生 Transformer 的标准 logit 级蒸馏，在通用语料上训练——直接检验 PARSE 的手术式方法是否优于最直接的小模型创建方式
7. 原始 Qwen3.5-0.8B：未压缩基线
8. Qwen2.5-0.1B（如有）：同家族已有小模型作为从头训练参考点
...
**A1. CIT 组成消融**：对比完整 CIT（激活+梯度，$\alpha=0.6$）vs 纯激活（$\alpha=1.0$）vs 纯梯度（$\alpha=0.0$）
...
基于 ShortGPT [7] 已有发现和 CIT 方法论，预期 CIT 将呈现"能力悬崖"模式：浅层（0–5）主要贡献表层语言特征；中间层（6–15）在学科和场景间均匀分布；深层（16–23）所有能力维度上 CIT 得分成比例积累，源于其在残差计算图中的位置。各能力轴展现的是定性不同的重要性轮廓（模块化）还是仅共享轮廓的幅度差异（集中化），是 CIT 框架设计的待解实证问题。

---

## 5. 假说与预期结果

以下假说源自上述方法论框架，有待通过第 4 节描述的管线进行实证验证。

### 5.1 核心假说

**假说 1（能力特定保持）**。基于保留剖面加权的 CIT 层选择，预期比能力无关方法（Wanda [5]、SparseGPT [4]、LayerDrop [8]）在相同参数预算下取得更高保留能力 CRR。随保留剖面收窄，优势预期增大。

**假说 2（层重要性结构）**。CIT 分析将揭示 LLM 能力遵循模块化模式还是集中模式。答案直接影响能力保持型压缩的选择性精度。因子化分析 (A6) 将量化三维结构在乘法近似下保留了多少信息。

**假说 3（DCR 开销）**。DCR 仅 0.08M 参数，预期相比独立部署剖面模型引入极小的交叉能力干扰。辅助分类损失 $\mathcal{L}_{cls}$ 提供弱监督以鼓励 DCR 学习能力判别性路由。
...
若上述假说得到确认：
1. **LLM 的能力结构**：高跨轴相关支持集中假说——能力沿深度单调递增分布；低相关支持模块化假说。
2. **FFN 冗余性范围**：在跨维度上成功的 No-FFN 移植将扩展 Needle [54] 的 FFN 冗余性原理。
3. **路由效率**：0.08M 参数的 DCR 有效路由将证明参数高效共享路由可替代独立专门模型。

### 5.3 局限

1. **实证验证待完成**。第 4 节实验方案已实现但尚未执行。本节所有结果讨论均为假说。下一步工作是执行管线并报告实证结果。

2. **CIT 因子化在高相关下的局限**。乘法因子化（式 4）隐含轴贡献近似正交的假设。若跨轴相关较高（已有层重要性研究 [7] 提示如此），因子化可能将三维分解退化为单深度加权排序。完整（非因子化）CIT 计算和对比 CIT 变体分别用于诊断和缓解此局限。

3. **参数预算与压缩比澄清**。常引用的 8.8× 压缩目标（752M→85M）需澄清：FFN 约占 65% 参数，即使移除全部 FFN 也仅剩 ~263M（35% × 752M），仅 2.86× 压缩。达到 8.8× 需额外措施，如保留层中的注意力头剪枝或更激进的层移除。实际可达压缩比是在特定保留剖面下的实证量。

4. **单一架构**。CIT 方法原则与架构无关，但目前仅与 Qwen 系模型集成。Llama、Mistral、Gemma 等架构验证对泛化性至关重要。

5. **标定数据规模**。当前每能力 15 条样本提供紧凑诊断探针，但可能遗漏稀有但关键的能力输入。

6. **DCR 表达力**。当前 DCR 使用单线性投影，限制为全局路由决策。多头路由或层级门控等更具表达力的架构可能以额外参数为代价实现更精细控制。

7. **长序列稳定性**。DCR 门控调制从均值池化嵌入计算，可能无法捕获长上下文中的位置依赖能力需求。

8. **轴选择依据**。19 轴分解基于已有评估基准 [56,57] 和部署场景 [25,26,27]，但具体轴选择（尤其在学科轴内）仍是设计决策。轴粒度敏感度留待消融。其他分解方式（如按语言复杂度、领域或格式）可能捕获当前轴未体现的正交变差。

### 5.4 总结

PARSE 框架以"该保留什么、该替换什么"的精准思维，重新定义了模型压缩的目标函数——从全局稀疏优化转变为三维能力保持问题。CIT 为量化层级能力贡献提供了原则性机制，DCR 使单一压缩模型可在不使用权重切换的情况下服务多个保留剖面。本文给出了 DCR 训练算法的完整规格，明确了因子化假设与局限，透明讨论了压缩比构成。完整的四阶段管线已作为开源软件实现，12 组保留剖面上的实证验证是下一步工作的直接目标。
...
[48] Anonymous. "Task-specific compression for large language models." *arXiv:2306.05685*, 2023.

[49] Anonymous. "Compact language models via priming and pruning." *arXiv:2406.09246*, 2024.
...
[53] Anonymous. "Regularizing towards well-calibrated large language models." *arXiv:2405.18654*, 2024.
