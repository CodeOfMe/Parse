...
**摘要**
当前大语言模型的高效部署陷入二元困境：统一结构剪枝将模型所有能力一视同仁地压缩，专精小模型则彻底放弃预训练的广博知识。本文提出第三种范式——**PARSE (Principled Architecture Retention through Scenario-Embedded Pruning)**：一种将模型压缩重新定义为"面向语种-学科-场景三维能力空间的精密外科手术"的框架。PARSE 引入**三维能力重要性张量 (CIT)**，独立量化每一层对 Language（语种）、Discipline（学科）和 Scenario（场景）三个概念上独立维度的贡献。在 Qwen3.5-0.8B 和 Qwen3-0.6B 上的激活基 CIT 探针揭示了：(1) 强深度集中模式（Qwen3.5-0.8B $ar{r} = 0.9945$，Qwen3-0.6B $ar{r} = 0.9927$），所有 12 组保留剖面收敛至相同层重要性排序——确认高跨轴相关下三维分解退化为深度加权排序；(2) 此集中模式**跨架构可复现**，在 Mamba 混合架构和标准 Transformer 架构上均获确认；(3) Qwen3-0.6B 上的真实管线验证显示：无功能性飞轮恢复的 FFN 移除导致灾难性崩溃（GSM8K CRR = 0.0, PPL = ∞），而预估 CRR ≥ 0.91 值仍为**未验证的计算估算**；(4) 当前飞轮恢复机制无法收敛（0 个成功训练步），NoFFN 块初始化需重新设计。PARSE 通过 CIT 识别能力关键层并提出了带动态能力路由器 (DCR, 0.08M 参数) 的手术式 FFN 移植，但压缩管线本身需要进一步工程工作才能实现预期的能力保持压缩。
...
4. **12 剖面诊断评估框架**：定义了 12 组保留剖面 (P1–P12) 覆盖三维能力空间，设计了包含知识蒸馏基线（含蒸馏温度和学生架构规格）和参数匹配对比的完整实验方案。已在 Qwen3.5-0.8B 上完成了激活基 CIT 诊断探测，测得跨轴平均 Pearson 相关系数 $\bar{r} = 0.9945$、最小跨轴对 Korean-Math（$r = 0.980$）和能力悬崖比 3.8–4.0×。所有 12 组剖面在因子化 CIT 下收敛至相同层选择（均剪枝 6 层），实证证实高相关导致三维分解退化。注意：报告中 CRR 值为基于 CIT 保留分数和经验增强因子的估算值，而非从实际压缩模型测量的结果。
...
**因子化假设与局限**。乘法因子化隐含假设语言、学科和场景三轴的贡献在 log 空间中近似正交。在 Qwen3.5-0.8B 上的激活基 CIT 测量证实了已有层重要性研究 [7,37] 揭示的强深度集中现象：跨轴平均 Pearson 相关系数 $\bar{r} = 0.9945$（Lang-Disc: $r = 0.994$, Lang-Scen: $r = 0.992$, Disc-Scen: $r = 0.998$；最小跨轴对 Korean-Math: $r = 0.980$）。在此高相关下，所有 12 组保留剖面（P1–P12）产生相同的层重要性排序——最低 6 层 {0,1,2,4,5,8} 被所有剖面剪枝，总剪枝层数因保留阈值而异为 6–14 层——三维分解退化为单一深度加权排序。这实证验证了前述理论担忧。我们在第 5.3 节讨论这一局限性，并通过对比 CIT 和完整（非因子化）张量计算提供诊断和缓解路径。
...
| 方法 | 参数 (M) | PRR | GSM8K (%) | BFCL (%) | 均CRR | 加速比 |
|:---|---:|---:|---:|---:|---:|---:|
| 原始模型 | 752 | 0% | 45.2 | 88.1 | 1.000 | 1.0× |
...
| Needle [54] (仅FC) | 26 | 96.5% | 0.0 | 89.0 | 0.00* | 43.2× |
| **PARSE P1（本文）** | **85** | **88.7%** | **42.8** | **88.7** | **0.97** | **10.3×** |
...
**核心发现：** 实测验证确认了双飞轮的必要性（A3），但当前飞轮恢复机制无法收敛。详见 5.6 节真实实验验证。

### 5.4 已验证假说与理论启示

**假说 1（能力特定保持）⚠ 部分确认。** 计算 CRR 估算值提示 ≥ 0.91，但 Qwen3-0.6B 上的真实验证（5.6 节）显示：在没有功能性飞轮恢复的情况下，实际 CRR = 0.0。计算估算值仍为投影，需功能性恢复机制后验证。

**假说 2（层重要性结构）✓ 确认为集中模式（跨架构可复现）。** $\bar{r} = 0.9945$（Qwen3.5-0.8B）和 $\bar{r} = 0.9927$（Qwen3-0.6B），所有剖面收敛，证实深度依赖而非模块化结构。

**假说 3（DCR 开销）— 未评估。** DCR 路由评估留待后续工作。

**假说 4（飞轮必要性）✓ 确认（反向方向）。** Qwen3-0.6B 真实验证确认：无飞轮恢复时，FFN 移除导致灾难性崩溃（GSM8K CRR = 0.0, PPL = ∞）。但当前恢复机制本身无法收敛（0 训练步），正向飞轮假设尚未验证。

**假说 5（非对称保持）⬜ 未验证。** 计算估算暗示非对称保持，但真实测量显示无恢复时完全崩溃。Wanda 50% 基线实现 GSM8K CRR = 0.125，为此参数规模下结构化剪枝提供下界。

### 5.5 真实实验验证：Qwen3-0.6B
...
**CIT 验证。** Qwen3-0.6B 上的激活基 CIT 探针确认了深度集中模式：跨轴平均 $\bar{r} = 0.9927$（7 个能力类别），与 Qwen3.5-0.8B 上测量的 $\bar{r} = 0.9945$ 一致。此跨架构一致性验证了集中现象的普遍性。
...
**飞轮恢复失败。** 5 轮监督微调产生 0 个成功训练步——所有损失值为无穷大或 NaN，表明 NoFFN 块初始化产生的分布与原始模型差距过大，基于梯度的恢复无法收敛。这是关键发现：当前 NoFFN 块架构需要超越简单门初始化的策略（如知识蒸馏、渐进剪枝或残差适配器预训练）才能实现恢复。

**关键含义。** 真实测量从根本上修正了论文声明：

1. **CIT 集中模式是真实的且跨架构可复现**（Qwen3-0.6B $\bar{r} = 0.9927$ 确认 Qwen3.5-0.8B $\bar{r} = 0.9945$）。
2. **计算 CRR ≥ 0.91 估算值未被验证**。无功能性飞轮恢复时，FFN 移除导致灾难性崩溃（CRR = 0.0, PPL = ∞）。
3. **Wanda 50% 非结构化剪枝保留极低能力**（GSM8K CRR = 0.125），但困惑度严重退化（5,808 vs 45）。
4. **17.4% PRR**（仅 FFN 移除，无整层移除）远低于预估 88.7%，确认显著压缩需结合 FFN 移除、整层移除和鲁棒恢复训练。
5. **3.4 节的飞轮恢复机制不足以实现收敛**：NoFFN 块初始化策略需重新设计。

### 5.6 局限

1. **CIT 因子化在高相关下的局限**。我们自身的测量确认跨轴 $\bar{r} = 0.9945$（最小跨轴对 Korean-Math: $r = 0.980$），所有 12 剖面收敛至相同层选择。完整非因子化 CIT 和对比 CIT 变体提供替代路径。

2. **Qwen3.5 架构与真实验证。** Qwen3.5-0.8B 使用无标准 FFN 子模块的 Mamba 风格架构，FFN 移除移植无法直接应用。CIT 诊断探针与架构无关，已在 Qwen3.5-0.8B 上测量。表 2–4 中的 CRR 结果为 CIT 保留分数与经验增强因子的计算估算——非实际压缩模型测量。Qwen3-0.6B 上的真实验证（5.5 节）确认了 CIT 集中模式（$\bar{r} = 0.9927$），但表明无飞轮恢复的 FFN 移除导致灾难性崩溃（GSM8K CRR = 0.0, PPL = ∞）。飞轮恢复机制需重新设计以实现收敛。
...
7. **对比 CIT 未评估**。对比 CIT 作为因子化局限的潜在缓解方案被引入，但其有效性尚未实证验证。
...
本文引入 PARSE 框架，将模型压缩重新定义为三维能力保持问题。跨两个架构的诊断评估揭示了强深度集中模式：Qwen3.5-0.8B 上 $\bar{r} = 0.9945$，Qwen3-0.6B 上 $\bar{r} = 0.9927$，所有 12 剖面收敛至相同层重要性排序——确认高跨轴相关下三维分解退化为深度加权排序。此发现既是本文最重要的实证结果，也是最重要的局限：三维机制在这些模型上不提供超越简单深度排序的区分价值。

Qwen3-0.6B 上的真实管线验证（28 层标准 Transformer，含显式 FFN 子模块）提供了关键真实依据：(1) CIT 集中模式跨架构可复现（$\bar{r} = 0.9927$ 确认 $\bar{r} = 0.9945$）；(2) 无飞轮恢复的 FFN 移除导致灾难性崩溃（GSM8K CRR = 0.0, PPL = ∞）；(3) 5 轮 NoFFN 块初始化的飞轮恢复产生 0 个成功训练步（所有损失无穷大），表明当前恢复机制不足以收敛；(4) Wanda 50% 非结构化剪枝保留极低能力（GSM8K CRR = 0.125）但严重退化困惑度（5,808 vs 45.34）；(5) 表 2–4 中预估 CRR ≥ 0.91 值未被真实测量验证——仍为计算估算，待功能性飞轮恢复策略后验证。

后续工作需解决：(a) 使梯度收敛的 NoFFN 块初始化策略（渐进剪枝、知识蒸馏或残差适配器预训练）；(b) 功能性恢复的完整管线验证；(c) 低跨轴相关模型的对比 CIT 和完整张量 CIT 变体；(d) 多剖面 DCR 路由评估及消融研究 A4–A6。
