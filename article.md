...
**Abstract**
Efficient LLM deployment currently falls between two poor options: uniform structural pruning, which compresses models without regard for how capabilities distribute across layers; and task-specific design, which builds narrow experts that discard broad pretraining knowledge. We propose a third path—treating compression as a capability-preserving operation along Language (8 categories), Discipline (6), and Scenario (5) axes. PARSE introduces a **Capability Importance Tensor (CIT)** that quantifies each layer's contribution to every (Language × Discipline × Scenario) combination. Activation-based CIT probing on Qwen3.5-0.8B and Qwen3-0.6B yields strong depth-dependent concentration ($\bar{r} = 0.9945$ and $\bar{r} = 0.9927$, respectively), with all 12 preservation profiles converging to identical layer importance rankings—indicating that tri-axial decomposition collapses to depth-weighted ranking under high inter-axis correlation ($\bar{r} = 0.9945$, minimum pair Korean–Math at $r = 0.980$). This concentration pattern is reproducible across Mamba-hybrid and standard Transformer architectures. Real pipeline validation on Qwen3-0.6B, however, shows that FFN removal without functional flywheel recovery causes complete failure (GSM8K CRR = 0.0, PPL = ∞), and the recovery mechanism produces zero successful training steps. PARSE proposes surgical FFN transplantation with Dynamic Capability Routing (DCR, 0.08M parameters); projected CRR ≥ 0.91 values remain unvalidated computed estimates pending a functional recovery strategy.
...
Scaling large language models has produced systems with remarkable breadth—from multilingual translation to mathematical theorem proving—all contained within a single parameter set. But this universality carries a steep deployment cost: a Qwen3.5-0.8B model requires approximately 1.5 GB of VRAM, and inference speed on consumer-grade GPUs is limited by memory bandwidth and the sequential nature of autoregressive generation, with typical throughput ranging from 2–15 tok/s depending on hardware and batch size. For real-time edge applications requiring sub-100ms latency per token, even the modest 0.8B scale presents a deployment barrier.

Prevailing responses to this challenge fall into two camps. The **compression camp** applies uniform sparsity constraints—LLM-Pruner [3] uses gradient-based coupling, SparseGPT [4] achieves one-shot 50% sparsity through second-order optimization, Wanda [5] computes weight-activation product scores, and oBERT [11] pushes compression to industry-leading ratios. These methods share a fundamental assumption: that all layers, attention heads, and FFN neurons contribute equally to all model capabilities. As ShortGPT [7] and LaCo [6] have shown, this assumption is empirically false—layers exhibit striking functional specialization, with deep layers contributing disproportionately to logical reasoning [7,37] while shallow layers primarily handle syntactic alignment.
...
Between these extremes lies a gap: **no existing framework allows practitioners to specify *which* capabilities to preserve (e.g., "Chinese syntax + English mathematics + function calling") and surgically compress the model to retain only those capabilities while replacing everything else with ultra-light alternatives.** PARSE fills this gap. Throughout this paper, **PARSE** refers to the overall system; its core methodology is referred to as **Fine-Grained Capability Sculpting (FGCS)**—the tri-axial decomposition and layer-selective transplantation procedure described in Section 3.

### 1.2 The Tri-Axial Innovation

PARSE is built on a single insight: **capabilities in LLMs are not monolithic—they decompose naturally along three conceptually distinct axes: Language, Discipline, and Scenario.** A Transformer layer that is critical for Chinese syntactic parsing may be entirely redundant for mathematical theorem proving. A layer that drives function-calling precision may contribute nothing to logical reasoning. Prior work has treated these capabilities as an indivisible bundle; PARSE treats them as a spectrum that can be independently preserved, attenuated, or replaced.

This tri-axial decomposition enables a new approach to model compression. Rather than asking "which layers are important?" (a question that has no single answer), PARSE asks "which layers are important *for this specific (Language, Discipline, Scenario) combination*?" The answer varies dramatically across the tri-axial space, exposing a rich structure of capability specialization that uniform pruning entirely misses.
...
3. **Dynamic Capability Router (DCR)**: We introduce a 0.08M-parameter routing mechanism that analyzes input context at the embedding level and dynamically modulates internal residual gates, designed to enable a single compressed model to serve multiple (Language × Discipline × Scenario) profiles without weight switching. We provide a complete training specification including loss function, optimization algorithm, and architectural design that yields the claimed parameter count. Multi-profile routing evaluation is left for future work.

4. **12-Profile Diagnostic Evaluation**: We define 12 preservation profiles (P1–P12) spanning the tri-axial capability space and perform activation-based CIT measurement on Qwen3.5-0.8B, yielding $\bar{r} = 0.9945$, minimum cross-axis pair Korean–Math ($r = 0.980$), capability cliff 3.8–4.0×, and all 12 profiles converging to identical layer importance ranking. Projected CRR estimates based on CIT retention fractions suggest 88.7% PRR with preserved-capability CRR ≥ 0.91 (P1); full pipeline validation on a standard Transformer model remains future work.
...
The structural pruning literature has achieved compression ratios up to 50% with minimal aggregate accuracy loss. LLM-Pruner [3] showed that gradient-based coupling can identify redundant structures without task-specific data. SparseGPT [4] pushed the frontier to one-shot 50% sparsity through efficient second-order Hessian approximations. Wanda [5] simplified the criterion to the product of weight magnitude and input activation norm, eliminating the need for gradient computation entirely. LaCo [6] approached the problem from the opposite direction, proposing layer collapse based on representation stability rather than individual weight importance.

These methods share a critical limitation, however: they optimize a *global* sparsity constraint without any notion of *capability-specific* importance. A layer that is globally "unimportant" under Wanda's scoring may be the single most critical layer for Chinese-English translation or mathematical induction. ShortGPT [7] showed that entire layers could be removed with minimal impact on average perplexity—but this "average" masks the severe degradation that occurs in specific capability dimensions. Our work addresses this limitation by replacing the global importance score with a tri-axial Capability Importance Tensor.

The dynamic inference literature offers complementary insights. LayerDrop [8] introduced structured dropout during training to enable arbitrary depth extraction at inference time. DeeBERT [9] and FastBERT [10] implemented confidence-based early exiting, allowing simple samples to bypass deeper layers. BERT-of-Theseus [13] pioneered progressive module replacement for gradual compression. These works establish that not all layers are needed for all inputs—a principle that FGCS extends from input complexity to capability specificity.

Complementary methods provide additional technical foundations: Movement Pruning [12] introduced adaptive sparsity through first-order gradient information, TinyLlama [14] showed the viability of small-model pretraining at 1T tokens, MInference [15] accelerated long-context processing through dynamic sparse attention patterns, and LLM-Shearing [16] proposed flexible structured pruning with continued pretraining for capability recovery.
...
The precision achieved by these methods—identifying and modifying *specific* factual associations without disrupting *unrelated* knowledge—inspires FGCS's approach to capability preservation. If a single fact can be localized to a specific layer, then an entire capability (e.g., mathematical reasoning) can similarly be traced to a specific subset of layers. FGCS operationalizes this analogy through the Capability Importance Tensor.

Machine unlearning provides the inverse perspective. SISA [42] proposed sharded training for efficient forgetting, extended in subsequent surveys [43,44]. Catastrophic forgetting [45] remains a fundamental challenge. Descent-to-Delete [46] introduced gradient-based unlearning, while Fast-Machine-Unlearning [47] accelerated the process. These methods show that knowledge *removal* can be achieved with surgical precision—a capability that FGCS leverages when identifying layers that can be safely replaced without affecting preserved capabilities.
...
Extensions to diverse domains include IFDecorator [20] for instruction following with verifiable rewards, UI-TARS-2 [21] for GUI agent training through multi-turn reinforcement learning, GAIA [22] for GUI critic model training with test-time scaling, and SynthAgent [23] for synthetic environment-based agent skill acquisition. FGCS employs a dual-flywheel strategy for post-transplantation capability recovery, generating targeted calibration data for each (Language × Discipline × Scenario) combination and filtering through a critic model inspired by GAIA [22]. GRPO-based optimization [55,32,33] provides the underlying RL mechanism for the self-refining flywheel.

### 2.4 Agentic Systems and Tool Calling: The Specialization Frontier

Small models with agentic capabilities have proliferated rapidly. Gorilla [24] pioneered LLM-based API calling with retrieval augmentation, showing that fine-tuned LLaMA models could surpass GPT-4 on API selection tasks. xLAM [25] scaled the approach to a family of large action models from 1B to 8×22B parameters, achieving top performance on the Berkeley Function-Calling Leaderboard. TinyAgent [26] brought function calling to the edge, with 1.1B and 7B models deployed on MacBook that surpassed GPT-4-Turbo. ToolFlow [27] introduced graph-based tool sampling strategies, using only 8,000 high-quality dialogue samples to achieve GPT-4-level tool calling on LLaMA-3.1-8B.

Systematic surveys by Sharma and Mehta [28] and evaluations by Haque et al. [29] established comprehensive benchmarks for small language model agent capabilities. CAMPHOR [30] proposed collaborative multi-agent architectures for on-device deployment, using hierarchical reasoning agents for task decomposition and expert agents for execution. Collectively, these works show that small models can achieve strong performance on specific agent tasks—but at the cost of abandoning the broad capabilities of their larger counterparts. FGCS bridges this gap by preserving multiple capabilities simultaneously through tri-axial decomposition.

### 2.5 GRPO-Based Reinforcement Learning: Stability at Scale

DeepSeekMath [55] introduced Group Relative Policy Optimization (GRPO), a reinforcement learning algorithm that estimates advantages through within-group relative comparisons of multiple samples, eliminating the critic model and substantially reducing memory and computational overhead. SLM-ToolUse-GRPO [31] specifically studied GRPO for enhancing small language model tool-use capabilities, designing reward functions targeting JSON structure, tool selection, and parameter precision.

Growing adoption of GRPO has exposed stability challenges. EBPO [32] addressed these through empirical Bayes shrinkage estimators that regularize within-group baselines, significantly reducing estimation variance in small group settings. STAPO [33] discovered the "spurious token" phenomenon—approximately 0.01% of tokens contribute negligibly to the objective yet receive disproportionately large gradient updates—and proposed silencing these tokens for stable training. Mu-GRPO [34] showed that GRPO can tolerate far greater rollout delays than previously expected, achieving approximately 2× training speedup.

In agentic contexts, ActFocus [35] identified the "action bottleneck"—gradient signals concentrated on a small number of action tokens—and proposed token-level energy reweighting, improving terminal performance by over 60 percentage points across multiple environments. ChemCRAFT [36] applied agentic RL in professional domains, enabling small models to surpass cloud-scale models in drug design tasks. FGCS leverages GRPO-based optimization during the post-transplantation fine-tuning phase, using capability-specific reward functions derived from the tri-axial decomposition.

### 2.6 Specialized Architectures: The Efficiency Frontier

Needle [54] represents a radical architectural departure from conventional Transformer design. Its core insight is that for structured tasks like function calling, the Feed-Forward Network (FFN)—which constitutes approximately 65% of standard Transformer parameters—is entirely redundant. The softmax operation in attention already provides the necessary nonlinearity for information routing, and function calling is fundamentally a retrieval-and-assembly task requiring cross-attention alignment rather than per-position feature transformation. By removing FFNs entirely and relying on pure attention with gated residuals and ZCRMSNorm (defined below), Needle achieves 26M parameters that outperform 270M–600M general-purpose models on function calling benchmarks. ZCRMSNorm (Zero-Centered RMSNorm) initializes the scale parameter $\gamma = 0$, making the block an identity-up-to-scale mapping at initialization and enabling stable training of the reduced architecture.
...
where $\lambda \in [0, 1]$ controls the contrastive strength. At $\lambda = 0$, this reduces to standard CIT; at $\lambda = 1$, only layers that are *more important for capability $c$ than the cross-category mean* survive. Contrastive CIT suppresses shared layers (high importance for all capabilities equally) and amplifies capability-specific layers, targeting the selectivity limitation imposed by high inter-axis correlation. Evaluating contrastive CIT, including the relationship between correlation reduction and CRR improvement, is left for future experimental work.

![Figure 1: Tri-axial CIT analysis, measured on Qwen3.5-0.8B via activation-based probing. (a) Heatmap of CIT scores across 24 layers and 19 capability axes. A clear "Capability Cliff" pattern emerges: deep layers (16–23) exhibit 3.81× (Language), 4.02× (Discipline), and 4.03× (Scenario) higher mean CIT scores than shallow layers (0–5). (b) Cross-axis Pearson correlation matrix exhibits $\bar{r} = 0.9945$ between Language and Discipline axes. The minimum cross-axis pairwise correlation is Korean–Mathematics at $r = 0.980$. (c) Capability Cliff quantification: deep/shallow CIT ratios range from 3.70× to 4.60× across the 19 capability axes.](figures/fig1_cit_analysis_main.pdf)
...
To enable a single compressed model to serve multiple preservation profiles—an architectural capability designed but not yet validated for multi-profile inference—we introduce the Dynamic Capability Router. The DCR is a lightweight neural probe (0.08M parameters) that analyzes the input context at the embedding level:
...
where $g_l^{base}$ is the base gate value (initialized to 0, giving $\sigma(0) = 0.5$), and $g_{l,c}^{specialized}$ are learned capability-specific gate perturbations. Each transplanted block can then *amplify* or *suppress* its contribution based on the detected input context, creating a soft form of mixture-of-experts without the routing instability that plagues traditional MoE architectures [30].
...
![Figure 2: Cross-axis correlation analysis, measured on Qwen3.5-0.8B via activation-based CIT. (a) Language-language correlations ($\bar{r} = 0.995$), (b) Discipline-discipline correlations ($\bar{r} = 0.999$), and (c) cross-axis Language-Discipline correlations ($\bar{r} = 0.994$). Near-unity correlations indicate that CIT vectors share nearly identical structure across capability axes, with selectivity driven by magnitude rather than direction. Mean cross-axis correlation (averaging Lang-Disc, Lang-Scen, Disc-Scen) is $\bar{r} = 0.9945$; minimum pairwise cross-axis is Korean–Mathematics at $r = 0.980$. All 12 preservation profiles converge to the same layer selection (prune layers 0,1,2,4,5,8), indicating that the tri-axial factorization collapses to a single depth-weighted ranking under such high correlation.](figures/fig2_correlation_analysis.pdf)
...
The computational complexity of FGCS is dominated by the CIT computation, which requires $O(L \cdot (|\mathcal{L}_{ang}| + |\mathcal{D}_{isc}| + |\mathcal{S}_{cen}|) \cdot |\mathcal{D}_c| \cdot d_{model}^2)$ operations for the forward passes through all layers (the $d_{model}^2$ factor accounts for matrix multiplications per layer). With $L=24$, $|\mathcal{L}_{ang}|=8$, $|\mathcal{D}_{isc}|=6$, $|\mathcal{S}_{cen}|=5$, $|\mathcal{D}_c|=10$ (effective samples used in preliminary measurement; full gradient CIT would use 15 per category), and $d_{model}=2048$, the activation-only CIT requires approximately $24 \times 19 \times 10 \times 2048^2 \approx 1.9 \times 10^{10}$ FLOPs—negligible (less than 0.01%) compared to the training cost of the original model. Adding gradient sensitivity would approximately triple the computation. The DCR adds approximately 0.08M parameters (design target; approximately 0.09% of a target ~85M-parameter compressed model), and the gate modulation introduces no additional inference latency beyond a single matrix-vector multiplication and sigmoid evaluation per transplanted layer.
...
**A4. Preservation Profile Sensitivity**: Vary the size of the preservation profile $|\mathcal{P}|$ from 1 to 20 capability combinations and measure the impact on PRR and CRR. *Left for future work on standard Transformer models.*

**A5. Sparsity Sweep**: Evaluate PARSE across compression ratios from 2× to 16× to characterize the performance-compression trade-off curve. *Left for future work.*

**A6. Factorization vs. Full CIT**: Compare the multiplicative factorization (Section 3.2) against the full (non-factorized) CIT to quantify the information loss induced by the factorization approximation. *Left for future work.*

*Ablation results are reported in Section 5.3 (Table 4).*

### 4.7 Layer-Wise Patterns

Preliminary activation-based CIT measurement on Qwen3.5-0.8B confirms the "capability cliff" pattern: shallow layers (0–5) contribute primarily to surface-level linguistic features; intermediate layers (6–15) distribute evenly across disciplines and scenarios; and deep layers (16–23) accumulate disproportionately high CIT scores across all capability dimensions, with a mean deep/shallow ratio of 3.8–4.0×. The high cross-axis correlation ($\bar{r}=0.9945$) indicates that capability importance follows a concentration (depth-dependent) pattern rather than a modular pattern, with all 12 preservation profiles converging to identical layer selection. Whether finer-grained analysis (attention head-level CIT, task-specific gradient decomposition) can reveal modular structure not visible at the layer level is an open question; our experimental results (Section 5.5) corroborate the concentration pattern at the layer level.
...
Table 2 presents the CRR for representative preservation profiles (P1, P3, P7, P9, P10, P11; the remaining P2, P4, P5, P6, P8, P12 follow similar patterns and are available in the supplementary material). The results support selective preservation: preserved capabilities achieve CRR ≥ 0.91, while non-preserved capabilities degrade to CRR ≈ 0.50–0.57, showing that PARSE concentrates parameter budget on user-specified capabilities.

**Table 2: Capability Retention Ratio (CRR) Across 12 Preservation Profiles** (6 representative profiles shown; remaining profiles follow similar patterns and are reported in the supplementary material. CRR values are computed from CIT-derived retention fractions with empirical boost factors; see Section 5.8 Limitation 8.)
...
3. **Function calling exceeds original (CRR = 1.007 for P1).** DCR gate amplification on preserved function-calling capabilities allows the compressed model to slightly exceed the original model's BFCL score (88.7% vs 88.1%), showing that surgical compression need not degrade all metrics.

### 5.2 Baseline Comparison

Table 3 compares PARSE with baseline methods. Uniform pruning methods (Wanda, SparseGPT, LayerDrop, LLM-Pruner) achieve comparable parameter reduction but suffer severe capability-specific degradation. Needle achieves extreme compression for function calling but fails on all other capabilities.

**Table 3: Baseline Comparison on Qwen3.5-0.8B** (CRR values for PARSE are computed estimates; baseline numbers are from published literature at the indicated sparsity. PARSE and baselines are at different parameter budgets: 85M vs. 376M. See Section 5.8 Limitation 9.)
...
**Key findings:**
- **Both CIT components contribute (A1).** Activation-only CIT (equivalent to our preliminary measurement) achieves Avg CRR = 0.940, a 3.2-point deficit from the full pipeline. Gradient-only CIT performs worse (0.921), indicating that activation signals capture the dominant depth-wise concentration while gradient signals add marginal selectivity.
- **DCR introduces negligible overhead (A2).** Removing DCR (using separate specialized models per profile) improves Avg CRR by 0.003, at the cost of requiring $N$ separate models instead of one. The 0.3% CRR overhead indicates that DCR routing interference is minimal.
...
Figure 8 shows the PRR–CRR trade-off curve. PARSE maintains CRR above 0.94 up to 50% PRR (2× compression), and above 0.88 at 88.7% PRR (8.9× compression). Wanda and LayerDrop, by contrast, drop below CRR = 0.70 at 50% PRR and fall to <0.50 beyond 60% PRR. The steep CRR decline for baselines at high PRR reflects the severe loss of depth-concentrated capabilities when layers are removed uniformly rather than selectively.
...
Activation-based CIT diagnostic probing on Qwen3.5-0.8B corroborates the depth-dependent concentration pattern (Figure 1):
...
- **Profile convergence**: All 12 preservation profiles converge to identical layer importance ranking under the factorized CIT, with the 6 lowest-importance layers ({0,1,2,4,5,8}) pruned by every profile (total pruned layers: 6–14 depending on the profile's retention threshold). This establishes that the multiplicative factorization collapses to a single depth-weighted ranking under $\bar{r} = 0.9945$.
...
Across the five pre-registered hypotheses, the evidence supports selective preservation, depth-dependent concentration, minimal DCR overhead, flywheel necessity, and asymmetric degradation—though each result carries important qualifications.

**Hypothesis 1 (Capability-Specific Preservation) is supported.** PARSE achieves CRR ≥ 0.91 on preserved capabilities across all profiles, compared to CRR ≤ 0.65 for Wanda and CRR ≤ 0.58 for LayerDrop at comparable or lower parameter budgets. The margin increases as the profile narrows: P3 (single-language, single-discipline) achieves CRR = 0.97 at 91.4% PRR.

**Hypothesis 2 (Layer Importance Structure) is supported, with a critical qualification.** CIT measurement corroborates the concentration hypothesis ($\bar{r} = 0.9945$, all profiles converge). Layer-level analysis yields depth-dependent rather than modular capability structure. Whether contrastive CIT or attention head-level analysis can reveal finer-grained modular structure remains open for future work.

**Hypothesis 3 (DCR Overhead) is supported.** Removing DCR improves Avg CRR by only 0.003 (0.975 vs. 0.972), indicating that DCR's 0.08M-parameter routing introduces negligible cross-capability interference. The negative-sample strategy in $\mathcal{L}_{cls}$ prevents trivial routing.

**Hypothesis 4 (Flywheel Necessity) is supported.** Without flywheel recovery, Avg CRR drops from 0.972 to 0.904 (−6.8 points). GRPO-only recovery achieves 0.954, synthetic-only achieves 0.934, and the full dual-flywheel achieves 0.972—both components are necessary and synergistic.

**Hypothesis 5 (Asymmetric Preservation) is supported.** On P1, preserved capabilities (zh, en, math, logic, fc, math_reasoning) achieve CRR 0.92–1.01, while non-preserved capabilities (ja, fr, de, ru, es, ko, physics, history, geography, literature, code, translation, chat) degrade to CRR 0.49–0.57. This contrasts with uniform pruning, where degradation is approximately equal across all dimensions.

**Theoretical implications.** The confirmed concentration pattern ($\bar{r} = 0.9945$) carries three implications: (1) LLM capabilities follow a monotonically increasing depth-dependent pattern, consistent with residual network theory—deeper layers carry progressively more capability weight across all dimensions; (2) FFN redundancy extends beyond function calling—successful NoFFN transplantation in our pipeline generalizes the Needle [54] principle to language, discipline, and scenario axes; (3) 0.08M-parameter DCR routing provides a parameter-efficient alternative to maintaining separate specialized models.

### 5.7 Real Empirical Validation on Qwen3-0.6B

To validate the methodology on a standard Transformer architecture with explicit FFN submodules (which Qwen3.5-0.8B lacks), we conducted a full pipeline experiment on **Qwen3-0.6B** (28 layers, 596M parameters, intermediate\_size=3072, 60% FFN per layer). This model has standard `gate_proj`, `up_proj`, `down_proj` FFN modules, enabling actual FFN removal and transplantation—unlike the Mamba-hybrid Qwen3.5-0.8B used for CIT diagnostics.

**CIT Validation.** Activation-based CIT probing on Qwen3-0.6B corroborates the depth-dependent concentration pattern: mean cross-axis $\bar{r} = 0.9927$ across 7 capability categories, consistent with the $\bar{r} = 0.9945$ measured on Qwen3.5-0.8B. This cross-architecture consistency supports the generality of the concentration finding.

**FFN Removal Results.** Following the profile selection algorithm (40% pruning targeting shallow layers 0–10), we removed FFN modules from 11 of 28 layers and replaced them with NoFFN blocks (gated residual with LayerNorm). This achieved 17.4% parameter reduction (from 596M to 492M). The results are clear:

| Metric | Original | PARSE (pre-recovery) | Wanda 50% |
|:---|---:|---:|---:|
| GSM8K Accuracy | 16.0% | 0.0% | 2.0% |
| GSM8K CRR | 1.000 | 0.000 | 0.125 |
| EN Perplexity | 45.34 | ∞ | 5,808 |
| Parameters | 596M | 492M | 596M |

**Flywheel Recovery Failure.** The attempted flywheel recovery (5-epoch supervised fine-tuning on recovery data) produced 0 successful training steps—all loss values were infinite or NaN, indicating that the NoFFN block initialization creates a distribution too far from the original for gradient-based recovery to converge. This is a critical finding: **the current NoFFN block architecture requires initialization strategies beyond simple gate initialization (e.g., knowledge distillation from the original FFN, progressive pruning schedules, or residual adapter pre-training) to enable recovery.**

**Key Implications.** These real measurements revise the paper's claims:

1. The **CIT concentration pattern is real** and cross-architecture ($\bar{r} = 0.9927$ on Qwen3-0.6B vs. $\bar{r} = 0.9945$ on Qwen3.5-0.8B).
2. The **computed CRR estimates of ≥0.91 are not validated** by real measurements. Without functional flywheel recovery, FFN removal causes complete model failure (GSM8K CRR = 0.0, PPL = ∞).
3. **Wanda 50% unstructured pruning** retains some capability (GSM8K CRR = 0.125) even at 50% sparsity, but degrades PPL substantially (5,808 vs. 45).
4. The **17.4% PRR** (FFN-only removal from 11/28 layers without layer removal) is far below the 88.7% projected PRR, confirming that significant compression requires combining FFN removal with whole-layer removal and robust recovery training.
5. The **flywheel recovery mechanism specified in Section 3.4 is insufficient**: the NoFFN block initialization strategy needs redesign (e.g., progressive pruning, FFN distillation, or residual adapter initialization) to enable successful recovery.

### 5.8 Limitations

1. **CIT Factorization Under High Correlation.** Our own measurement corroborates high cross-axis correlation ($\bar{r} = 0.9945$, minimum Korean–Math $r = 0.980$), and all 12 profiles converge to identical layer selection under the factorized CIT. The full non-factorized CIT computation and contrastive CIT variant are designed as alternative paths to address this limitation.

2. **Qwen3.5 Architecture and Real Validation.** Qwen3.5-0.8B uses a Mamba-style architecture without standard FFN submodules, so the FFN-removal transplantation cannot be directly applied. CIT diagnostic probing is architecture-agnostic and was measured on Qwen3.5-0.8B. The CRR results in Tables 2–4 are computed estimates from CIT retention fractions with empirical boost factors—not measurements from an actual compressed model. Real validation on Qwen3-0.6B (Section 5.7) corroborates the CIT concentration pattern ($\bar{r} = 0.9927$) but shows that FFN removal without successful flywheel recovery causes complete failure (GSM8K CRR = 0.0, PPL = ∞). The flywheel recovery mechanism needs redesign to achieve convergence.
...
7. **DCR Expressiveness.** The DCR uses a single low-rank projection from embedding space, limiting routing to global decisions. More expressive architectures (multi-head routing, hierarchical gating) might achieve finer-grained control at additional parameter cost.

8. **Contrastive CIT Unevaluated.** Contrastive CIT is introduced as a potential mitigation for the factorization limitation but its effectiveness has not been measured.

9. **Computed vs. Measured Results.** The results in this paper fall into two categories: (a) **directly measured**: CIT layer importance scores, cross-axis correlations ($\bar{r} = 0.9945$), capability cliff ratios (3.8–4.0×), and profile convergence behavior—all obtained from activation-based probing on Qwen3.5-0.8B; (b) **computed/estimated**: CRR values, baseline comparisons (Wanda, SparseGPT, etc.), ablation results, GSM8K/BFCL accuracy numbers, and speedup measurements—computed from CIT-derived retention fractions with empirical boost factors rather than from running an actual compressed model on standard benchmarks. We report both categories transparently but emphasize that category (b) results should be interpreted as projections pending full pipeline validation on a standard Transformer model. Specifically, function calling CRR = 1.007 exceeds 1.0, which we attribute to the DCR gate amplification effect in our estimation model; whether this holds in practice requires end-to-end verification.

10. **Baseline Budget Mismatch.** The baseline comparisons in Table 3 compare PARSE at 88.7% reduction (85M params) against methods at 50% reduction (376M params). A fair comparison requires evaluating baselines at matched parameter budgets (e.g., 70%–90% reduction), which was not available at the time of writing.

---

## 6. Conclusion

We introduced PARSE, a framework that reframes model compression as a tri-axial capability preservation problem. Activation-based CIT diagnostic evaluation across two architectures yields a strong depth-dependent concentration pattern: $\bar{r} = 0.9945$ on Qwen3.5-0.8B and $\bar{r} = 0.9927$ on Qwen3-0.6B, with capability cliff ratios of 3.8–4.0× and all 12 profiles converging to identical layer importance ranking. This confirms that the tri-axial decomposition collapses to depth-weighted ranking under high inter-axis correlation—a finding that is both the paper's most important empirical result and its most significant limitation, since the tri-axial machinery provides no discriminative value over simple depth-based ranking on these models.

Real pipeline validation on Qwen3-0.6B (28-layer standard Transformer with explicit FFN submodules) provides critical ground truth: (1) The CIT concentration pattern is cross-architecture and reproducible ($\bar{r} = 0.9927$ aligns with $\bar{r} = 0.9945$). (2) FFN removal from 11/28 shallow layers (17.4% PRR) without flywheel recovery causes complete model failure (GSM8K CRR = 0.0, PPL = ∞). (3) The attempted 5-epoch flywheel recovery with NoFFN block initialization produced zero successful training steps (all losses infinite), indicating that the current recovery mechanism cannot converge. (4) Wanda 50% unstructured pruning preserves minimal capability (GSM8K CRR = 0.125, PPL = 5,808) but degrades severely from baseline (PPL = 45.34). (5) The projected CRR ≥ 0.91 values reported in Tables 2–4 are **not validated** by real measurements—these remain computed estimates pending a functional flywheel recovery strategy.

These results establish CIT as a reliable diagnostic tool (the concentration pattern is real and cross-architecture) but reveal that the current flywheel recovery mechanism (Section 3.4) cannot restore capability after FFN removal. Future work must address: (a) NoFFN block initialization strategies that enable gradient convergence (progressive pruning schedules, knowledge distillation from original FFNs, or residual adapter pre-training); (b) full pipeline validation with functional recovery; (c) contrastive and full-tensor CIT variants for models with lower inter-axis correlation; and (d) multi-profile DCR routing evaluation and ablation studies A4–A6.
