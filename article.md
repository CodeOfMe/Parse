...
**Abstract**
The prevailing paradigm for efficient LLM deployment oscillates between two unsatisfactory extremes: uniform structural pruning, which blindly compresses models without regard for the heterogeneous distribution of linguistic, disciplinary, and task-specific capabilities within Transformer layers; and task-specific architecture design, which builds narrow expert models that abandon the broad knowledge accumulated during pretraining. This work introduces a third paradigm: treating model compression as a *capability-preserving surgical procedure* operating at the intersection of language, discipline, and scenario dimensions. PARSE (Principled Architecture Retention through Scenario-Embedded Pruning) introduces a tri-axial **Capability Importance Tensor (CIT)** that independently quantifies each layer's contribution along three *designed to be distinct* (Cartesian product) axes: *Language* (8 categories), *Discipline* (6 categories), and *Scenario* (5 categories). Through this lens, PARSE identifies capability-critical layers for user-specified preservation profiles and surgically transplants redundant layers with ultra-efficient No-FFN attention blocks. A lightweight **Dynamic Capability Router (DCR)** (0.08M parameters) modulates internal residual gates based on real-time input context, enabling a single compressed model to serve multiple capability combinations without weight switching. Activation-based CIT probing on Qwen3.5-0.8B across 12 preservation profiles reveals: (1) strong depth-dependent concentration ($\bar{r} = 0.9945$, capability cliff 3.8–4.0×) with all 12 profiles converging to identical layer importance ranking under the factorized CIT—confirming that tri-axial decomposition collapses to depth-weighted ranking under high inter-axis correlation; (2) projected CRR ≥ 0.91 on preserved capabilities with 88.7% parameter reduction (752M→85M), compared to Wanda's CRR ≤ 0.65 and LayerDrop's CRR ≤ 0.58 at unmatched parameter budgets; (3) projected function calling CRR of 1.007, with non-preserved capabilities degrading to CRR ≈ 0.50. We emphasize that CRR and benchmark results are computed from CIT-derived retention fractions, not measured from an actual compressed model; full pipeline validation on a standard Transformer architecture is required. The complete four-stage pipeline is released as open-source software.ic evaluation. Preliminary activation-based CIT diagnostic probing was performed on Qwen3.5-0.8B (which uses a Mamba-style linear attention architecture; see Section 4.1), measuring a mean cross-axis Pearson correlation of $\bar{r} = 0.9945$ and a capability cliff ratio of 3.8–4.0× between deep and shallow layers. All 12 profiles converge to identical layer selection under the factorized CIT, empirically confirming that the tri-axial decomposition collapses to depth-weighted ranking under high inter-axis correlation. We present a comprehensive experimental design, specify evaluation metrics and baselines, and derive testable hypotheses from the methodological framework.
...
2. **Qwen3.5 Architecture Context.** Qwen3.5-0.8B uses a Mamba-style architecture without standard FFN submodules. CIT diagnostic probing (hidden state activations) is architecture-agnostic and was measured directly on Qwen3.5-0.8B. However, the FFN-removal transplantation pipeline and gradient sensitivity $G(l,c)$ are designed for standard Transformer architectures and cannot be directly applied to this model. The CRR results in this paper are **estimated** from activation-only CIT diagnostics combined with architecture-specific parameter accounting: compressed model sizes are derived from known layer parameter counts, and CRR values are computed from CIT retention fractions combined with empirical flywheel and DCR boost estimates. These are **not measurements from an actual compressed model**. Full validation requires running the complete transplantation and flywheel recovery pipeline on a standard Transformer model with explicit FFN submodules (e.g., Llama-3.2-1B, Qwen2.5-1.5B).
...
This paper introduced PARSE, a framework that reframes model compression as a tri-axial capability preservation problem rather than a global sparsity optimization. Activation-based CIT diagnostic evaluation on Qwen3.5-0.8B across 12 preservation profiles reveals a strong depth-dependent concentration pattern ($\bar{r} = 0.9945$, capability cliff 3.8–4.0×), with all 12 profiles converging to identical layer importance ranking under the factorized CIT—confirming that the tri-axial decomposition collapses to depth-weighted ranking under high inter-axis correlation. This finding is simultaneously the paper's most important empirical result and its most significant limitation: the tri-axial machinery provides no discriminative value over simple depth-based ranking on Qwen3.5-0.8B, though it may prove valuable for models with lower cross-axis correlation. Projected CRR estimates suggest 88.7% parameter reduction (752M→85M) with CRR ≥ 0.91 on preserved capabilities—compared to Wanda's computed CRR ≤ 0.65 at 50% sparsity (unmatched parameter budget). These projections require validation through full pipeline execution on a standard Transformer model with explicit FFN submodules.

The contrastive CIT and full-tensor computation are provided as alternative paths for profiles requiring finer-grained selectivity. Multi-profile DCR routing evaluation and full ablation studies (A4–A6) are left for future work.
